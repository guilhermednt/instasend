...
Desktop → server:
  ...
  register   Sent immediately after connect. Authenticates the client,
             declares its identity, and lists all currently stored hashes.
             The server validates ownership of each hash; any hash that is
             unowned or owned by a different client_id is retired and a new
             one is assigned via `assigned`.
             {type, version, token, client_id, hashes: [{hash, filename}]}
...
_FIELDS = {
    "register": ("version", "token", "client_id", "hashes"),
    "add":      ("filename",),
    "remove":   ("hash",),
